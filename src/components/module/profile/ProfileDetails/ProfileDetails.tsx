...
const ProfileDetails = ({
  ...
  data: TPapers[];
  currentUser: TResearchAssociate;
}) => {
  return (
    <section className="container mx-auto w-[90%]   my-[40px]    sm:px-6 lg:px-8">
        ...
        <div className="lg:w-2/3 p-8 lg:p-10 flex flex-col space-y-8">
          ...
          <div className="flex flex-col items-center sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
            ...
            <div className="flex space-x-4 ">
              {currentUser?.socialLinks?.google_scholar && (
                <a
                  href={currentUser.socialLinks.google_scholar}
                  target="_blank"
              ...
              {currentUser?.socialLinks?.researchgate && (
                <a
                  href={currentUser.socialLinks.researchgate}
                  target="_blank"
                  rel="noopener noreferrer"
      ...
      <div className=" my-3">
        <Tabs defaultValue="Pulications" className="w-full">
          <TabsList className="grid w-full grid-cols-2 rounded-none">
            <TabsTrigger className="rounded-none" value="Ongoing">
              Ongoing Project
            </TabsTrigger>
            <TabsTrigger className="rounded-none" value="Pulications">
              Pulications
            </TabsTrigger>
          </TabsList>
