...
const CountdownTimer = ({ 
    ...
    const updateTimer = () => {
      ...
      console.log('CountdownTimer Debug:', {
        ...
        timeUnits: {
          ...
          minutes: dayjs.duration(start.diff(now, "second"), "seconds").minutes(),
          seconds: dayjs.duration(start.diff(now, "second"), "seconds").seconds()
        }
      });
      ...
      const diffToStart = start.diff(now, "second");
         ...
         const timeLeftStr = `${totalDays.toString().padStart(2, '0')}:${totalHours.toString().padStart(2, '0')}:${totalMinutes.toString().padStart(2, '0')}:${totalSeconds.toString().padStart(2, '0')}`;
         ...
         if (daysUntilStart <= 30) {
           progressPercent = Math.max(0, Math.min(100, ((30 - daysUntilStart) / 30) * 100));
         } else {
           ...
           progressPercent = Math.max(0, Math.min(15, (1 / monthsUntilStart) * 15)); // Max 15% for far events
         }
  ...
  const TimeUnit = ({ value, label, isHighlighted = false }: { value: number; label: string; isHighlighted?: boolean }) => (
    <div className={`flex flex-col items-center ${isHighlighted ? 'scale-110' : ''} transition-transform duration-300`}>
      <div className={`relative w-16 h-16 rounded-xl flex items-center justify-center ${
        ...
        <span className={`text-lg font-bold ${isHighlighted ? 'text-white' : 'text-gray-800'}`}>
      ...
      <span className={`text-xs font-medium mt-2 ${isHighlighted ? 'text-brand-secondary' : 'text-gray-600'}`}>
        {label}
      </span>
    </div>
  );
  ...
  return (
    <div className={`${className}`}>
      {status !== "finished" && (
        <div className={`p-6 rounded-2xl ${getStatusBgColor()} border ${getStatusBorderColor()} shadow-lg`}>
          {/* Header */}
          ...
          <div className="grid grid-cols-4 gap-3 mb-4">
            ...
            <TimeUnit 
              ...
              label="Hours" 
              isHighlighted={timeUnits.days === 0 && timeUnits.hours <= 6 && timeUnits.hours > 0}
            />
            <TimeUnit 
              value={timeUnits.minutes} 
              label="Minutes" 
              isHighlighted={timeUnits.days === 0 && timeUnits.hours === 0 && timeUnits.minutes <= 30 && timeUnits.minutes > 0}
            />
            <TimeUnit 
              value={timeUnits.seconds} 
              label="Seconds" 
              isHighlighted={timeUnits.days === 0 && timeUnits.hours === 0 && timeUnits.minutes === 0}
            />
          </div>
           ...
           <div className="mb-3">
             ...
             <div className="text-xs text-gray-500 text-center mt-1">
               {status === "upcoming" 
                 ? progress < 15 ? "Event announced" : progress < 50 ? "Getting closer" : "Almost here!"
                 : "Event in progress"
               }
             </div>
           </div>
      ...
      {status === "finished" && (
        <div className={`p-6 rounded-2xl ${getStatusBgColor()} border ${getStatusBorderColor()} shadow-lg text-center`}>
          <div className="flex items-center justify-center gap-2 mb-3">
            <CheckCircle className="w-6 h-6 text-gray-500" />
            <span className="text-lg font-semibold text-gray-700">Event Completed</span>
          </div>
