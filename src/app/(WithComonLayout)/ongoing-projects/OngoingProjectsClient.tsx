...
const OngoingProjectsClient = () => {
  ...
  useEffect(() => {
    const fetchPapers = async () => {
      try {
        ...
        // Fetch only ongoing research papers
        const papersData = await GetAllResearchPaperPublicByStatus("ongoing");
        ...
        console.error("Error fetching ongoing papers:", err);
        setError("Failed to fetch ongoing research papers");
        setPapers([]);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading ongoing research papers...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div>
      <ResearchPapersPage papers={papers} initialStatus="ongoing" />
    </div>
  );
};
